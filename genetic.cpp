...
vector<chromosome_t> genetic_alg(int population_size, int chromosome_size, int iterations,
                        ...
                        function<vector<chromosome_t>(double, const chromosome_t &, const chromosome_t &)> crossover,
...
double one_max_function(chromosome_t c)
{
    ...
    //cout<< sum<<endl;
    auto result = tabu_search(generate_random_problem(10,20),40,sum,300,30,10);
    if(result.first > 0){
        return 100;
    }
    //cout<<result<<endl;
